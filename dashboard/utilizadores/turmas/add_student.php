...
if ($turmaId <= 0 || $alunoId <= 0) {
    util_set_flash('erro', 'Dados inválidos para adicionar aluno.');
    util_redirect(util_url('dashboard/utilizadores/turmas/index.php'));
}
...
if ((int) $turma['ativa'] !== 1) {
    util_set_flash('erro', 'Não pode adicionar alunos em turma arquivada.');
...
if ($assoc) {
    ...
    if ($assocTurmaId === $turmaId) {
        util_set_flash('erro', 'Este aluno ja esta nesta turma.');
    } else {
...
util_set_flash('sucesso', 'Aluno adicionado a turma com sucesso.');
util_redirect(util_url('dashboard/utilizadores/turmas/detail.php?id=' . $turmaId));
