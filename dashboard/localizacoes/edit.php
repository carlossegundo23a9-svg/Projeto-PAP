...
if (!$id) {
    ...
    try {
        ...
        if (!$local) {
            ...
            $values['nome'] = (string) $local['nome'];
            $values['descricao'] = (string) ($local['descricao'] ?? '');
        }
    } catch (Throwable $e) {
        $message = "Erro ao carregar localiza\u{00E7}\u{00E3}o.";
    }
}
...
if ($id && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!app_csrf_is_valid($_POST['csrf_token'] ?? null)) {
        ...
        if ($errors === []) {
            try {
                if ($repo->nomeExiste($values['nome'], (int) $id)) {
                    $errors[] = "J\u{00E1} existe uma localiza\u{00E7}\u{00E3}o com este nome.";
                } else {
                    $ok = $repo->atualizar((int) $id, $values['nome'], $values['descricao'] !== '' ? $values['descricao'] : null);
                    if (!$ok) {
                        $errors[] = "Localiza\u{00E7}\u{00E3}o n\u{00E3}o encontrada.";
                    } else {
                        $_SESSION['flash'] = [
                            'type' => 'sucesso',
                            'message' => "Localiza\u{00E7}\u{00E3}o atualizada com sucesso.",
                        ];
                ...
                $errors[] = "Erro ao atualizar localiza\u{00E7}\u{00E3}o. Verifique os dados e tente novamente.";
            }
        }
...
<div class="layout">
  ...
  <main class="content">
    <div class="detail-head">
      <div>
        <h1 class="page-title">Editar Localiza&ccedil;&atilde;o</h1>
    ...
    <section class="panel panel-form" id="painel-form-local">
      <form method="post" id="form-local">
        ...
        <div class="form-actions">
          <button class="btn btn-primary" type="submit">Guardar Altera&ccedil;&otilde;es</button>
        </div>
      </form>
    </section>
  </main>
